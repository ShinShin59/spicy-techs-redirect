...
function App() {
  ...
  const mobileMetadataOpen = useUIStore((s) => s.mobileMetadataOpen)
    ...
    <div className={`w-screen h-screen text-white flex flex-col overflow-hidden select-none relative ${lightweightMode ? "lightweight-mode" : ""}`}>
      ...
      <div className="relative z-10 flex flex-col flex-1 min-h-0">
        ...
        <div className="flex flex-1 min-h-0 gap-3 p-3 pt-[2em]">
          {!isMobile && <Metadata />}
          <div
            className={`flex-1 flex flex-col items-center min-h-0 overflow-auto px-4 ${isMobile ? "pt-1 gap-2 pb-24" : "pt-0 pb-4 mt-48 gap-4"}`}
          >
            <BuildLayout
              ...
              operations={
                panelVisibility.operationsOpen ? (
                  <Operations
                    variant="row"
                  />
                ) : undefined
      ...
      {isMobile && (
        <Drawer
          ...
          aria-label="Metadata, settings and builds"
        >
          <div
          ...
          >
            <div
              className={`flex-1 min-h-0 overflow-hidden shrink-0 flex flex-col ${
                isPortrait ? "" : "w-1/2 overflow-y-auto"
              }`}
            >
              <Metadata inDrawer />
            </div>
            <div
              className={`flex-1 min-h-0 shrink-0 flex flex-col ${
                isPortrait
                  ? "overflow-y-auto border-t border-zinc-700/50"
                  : "w-1/2 overflow-y-auto border-l border-zinc-700/50"
              }`}
            >
              <BuildsSidebar inDrawer />
            </div>
          </div>
        </Drawer>
      )}
